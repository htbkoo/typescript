...
var KthLargest = function (k: number, nums: number[]) {
    this._k = k;
    this._heap = nums.reduce((heap, num) => heap.push(num), KthLargestHeap.newHeap());
    return this;
};
...
KthLargest.prototype.add = function (val: number): number {
    this._heap.push(val);
    let k = this._k;
    let heap = KthLargestHeap.copyOf(this._heap);
...
export class KthLargestHeap {
    ...
    public static copyOf(heap: KthLargestHeap): KthLargestHeap {
        return new KthLargestHeap(heap._nums.slice());
    }
