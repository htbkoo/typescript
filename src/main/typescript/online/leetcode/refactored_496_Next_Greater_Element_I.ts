...
const nextGreaterElement = function (nums1: number[], nums2: number[]): number[] {
    ...
    return nums1.map(toNextGreaterElements);
    ...
    function buildNextGreaterElements() {
        ...
        _.rangeRight(lengthNums2 - 1).forEach(index => {
            ...
            if (stack.length !== 0) {
                nextGreaterElements[element] = stack[stack.length - 1];
            }
            stack.push(element);
        });
    ...
    function toNextGreaterElements(num) {
        return _.defaultTo(nextGreaterElements[num], NO_GREATER_ELEMENT);
    }
};
