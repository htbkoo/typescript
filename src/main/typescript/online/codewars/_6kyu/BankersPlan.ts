...
/*
 https://www.codewars.com/kata/bankers-plan/train/typescript
...
export class G964 {
    public static fortune(f0:number, p:number, c0:number, n:number, i:number):boolean {
        // your code
        // O(n) naive approach
        var yr = 0;
        p = p / 100;
        i = i / 100;
        for (; yr < n; ++yr) {
            f0 = Math.floor(f0 * (1 + p) - c0);
            if (f0 < 0) {
                return false;
            }
            c0 = Math.floor(c0 * (1 + i));
        }
        return f0 >= 0;
    }
}