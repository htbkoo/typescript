...
/*
 https://www.codewars.com/kata/color-choice/train/typescript
...
export class G964 {
    ...
    public static checkchoose(m:number, n:number):number {
        // your code
        if (m === 1) {
            return 0;
        }
        ...
        var half:number = Math.floor((n + 1) / 2);
        var x:number = n;
        for (let i = 2; i <= half; ++i) {
            if (x > m) {
                return -1;
            }
            x = Math.floor(x * (n + 1 - i) / i);
            if (x === m) {
                return i;
            }
        }
